...
@Component({
  ...
  template: `
    <div class="card">
      <h3>Cart publisher (from remote-cart)</h3>
      <p>Click a product â€” this remote publishes a <code>cart:item-added</code> event. The host's cart badge subscribes and updates without any direct coupling.</p>
      <div class="products">
        @for (p of products; track p.sku) {
          <button class="product" (click)="add(p.sku, p.qty)">
            <strong>{{ p.name }}</strong>
            <span>SKU {{ p.sku }} Â· +{{ p.qty }}</span>
          </button>
        }
      </div>
      @if (lastSent(); as last) {
...
export default class CartPublisherComponent {
  ...
  add(sku: string, qty: number): void {
    this.bus.publish<CartAdded>('cart:item-added', { sku, qty }, { source: 'cart/CartPublisher' });
    this.lastSent.set(`${sku} Ã— ${qty}`);
  }
