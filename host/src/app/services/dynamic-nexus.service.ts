...
import { RegistryWebSocketService } from './websocket.service';
...
export class DynamicNexusService {
  ...
  async initialize(): Promise<void> {
    ...
    // 1. Initial fetch via HTTP (registry/cache/static backup fallback chain)
    try {
  ...
  private async registerRoute(remote: RemoteConfig): Promise<void> {
    try {
      ...
      const component = moduleRef.EntryComponent ?? moduleRef.default ?? moduleRef[Object.keys(moduleRef)[0]];
      if (!component) {
      ...
      console.log(`[nexus] Registered "${remote.name}" at /${remote.routePath} (from ${remote.url})`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      console.error(`[nexus] Failed to load remote "${remote.name}" from ${remote.url}:`, msg);
